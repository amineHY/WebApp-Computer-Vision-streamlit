...
class Object_Detection_YOLO:
    def __init__(self, paramYolo):
        ...
        self.net = self.load_YOLO_model(self.modelCfg, self.modelWeights)
    ...
    def load_YOLO_model(self, modelCfg, modelWeights):
        ...
        if os.path.exists(modelCfg) and os.path.exists(modelWeights):
            net = cv.dnn.readNetFromDarknet(cfgFile=modelCfg, darknetModel=modelWeights)
        else:
    ...
    def draw_bbox(self, frame, classId, conf, box, color):
        ...
        cv.rectangle(
            img=frame, pt1=(left, top), pt2=(right, bottom), color=color, thickness=3
        )
        ...
        cv.rectangle(
            frame,
            pt1=(left, top - round(1.5 * label_height)),
            pt2=(left + round(1.5 * label_width), top + baseLine),
            color=colorWhite,
            thickness=cv.FILLED,
        )
        cv.putText(
    ...
    def postprocess(self, frame, outs):
        ...
        for i in indices:
            ...
            self.draw_bbox(
                frame, classIds[i], confidences[i], box, self.colors[classIds[i]]
            )
    ...
    def run(self, frame, motion_state):
        ...
        self.allConfidence.append(100 * np.around(self.saveConfidences, decimals=2))

        # Create a dataframe
        dataframe_plugin = pd.DataFrame(
            data=list(
                zip(
                    self.allNumber_of_detections,
                    self.allDetected_object,
    ...
    def display_summary(self, ph, output_result):
        ...
        if output_result["displayFlag"]:
            ...
            ph[1].markdown(
                "* Number of detected objects :\t {}".format(
                    output_result["number_of_detections"][-1]
                )
            )

            ph[2].markdown(
                "* Detected objects  :\t {}".format(
                    output_result["detected_object"][-1]
                )
            )

            ph[3].markdown(
                "* Detections probabilities (%):\t {}".format(
                    output_result["confidence"][-1]
                )
            )
        else:
            pass
...
class Object_Detection_MobileNetSSD:
    ...
    def draw_bbox(self, frame, classId, conf, box, color):
        ...
        cv.rectangle(frame, (startX, startY), (endX, endY), color, thickRect(self.H))

        # Display the label at the top of the bounding box
        label = "{}:{:.2f}".format(classId, conf * 100)
        labelSize, baseLine = cv.getTextSize(label, cv.FONT_HERSHEY_SIMPLEX, 0.45, 1)

        startY = max(startY, labelSize[1])

        cv.rectangle(
            frame,
            (startX, startY - round(1.5 * labelSize[1])),
            (startX + round(1.5 * labelSize[0]), startY + baseLine),
            colorWhite,
            cv.FILLED,
        )
        cv.putText(
            frame, label, (startX, startY), fontFace, 0.65, colorBlack, thickText
        )
    ...
    def run(self, frame, motion_state):
        ...
        if motion_state:
            ...
            # Input blob
            blob = cv.dnn.blobFromImage(
                cv.resize(frame, (300, 300)), 0.007843, (300, 300), 127.5
            )
...
class Face_Detection:
    ...
    def draw_bbox(self, frame, classId, conf, box, color):
        ...
        cv.putText(
            frame, label, (startX, startY), fontFace, 0.75, colorBlack, thickText
        )
    ...
    def run(self, frame, motion_state):
        ...
        if motion_state:
            ...
            for i in range(0, detections.shape[2]):
                ...
                if confidence > self.confThresh:
                    ...
                    self.draw_bbox(bboxed_frame, "face", confidence, bbox, colorGreen)
        ...
        # Create a dataframe
        dataframe_plugin = pd.DataFrame(
            data=list(zip(self.allNumberOfFaces)), columns=["Nbr Det Faces"]
        )
...
class Motion_Detection:
    ...
    def load_model(self, method):
        ...
        """
        if method == "knn":
            model = cv.createBackgroundSubtractorKNN(100, 400, True)
        return model
    ...
    def run(self, frame):
        ...
        self.output_result = dict(motion_state=flag, mask=mask, pixel_count=pixelCount)
